...
const criarItemDeAtividade = (atividade) => {
  ...
  <div class="card-bg">
      ...
      <div>
          ...
          <time class="full">
              Em <strong>${atividade.local}</strong>,
...
const salvarAtividade = (event) => {
  ...
  const local = dadosDoFormulario.get('local')
...
const criarLocalSelecao = () => {
  const locais = ['São Paulo', 'Rio de Janeiro', 'Belo Horizonte', 'Curitiba', 'Salvador'];
  let locaisSelecao = '';

  for (let local of locais) {
      locaisSelecao += `<option value="${local}">${local}</option>`;
  }

  document
      .querySelector('select[name="local"]')
      .innerHTML = locaisSelecao;
}
criarLocalSelecao();
